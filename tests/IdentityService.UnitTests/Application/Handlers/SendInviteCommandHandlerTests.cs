...
public sealed class SendInviteCommandHandlerTests
{
        ...
        SendInviteCommandHandler handler) CreateHandler()
    {
        ...
        var handler = new SendInviteCommandHandler(
            orgRepo, inviteRepo, userRepo, emailService, unitOfWork, outboxRepo, mapper,
            NullLogger<SendInviteCommandHandler>.Instance);
    ...
    public async Task Handle_Throws_WhenOrganizationNotFound()
    {
        ...
        orgRepo.GetByIdAndUserIdAsync(Arg.Any<Guid>(), Arg.Any<Guid>(), Arg.Any<CancellationToken>())
    ...
    public async Task Handle_Throws_WhenRequesterIsNotMember()
    {
        ...
        // The repository filters membership at the DB level, so a non-member
        // gets null back even though the organization exists.
        orgRepo.GetByIdAndUserIdAsync(org.Id, ownerId, Arg.Any<CancellationToken>()).Returns(org);
        orgRepo.GetByIdAndUserIdAsync(org.Id, strangerId, Arg.Any<CancellationToken>()).Returns((Organization?)null);

        var command = new SendInviteCommand(
            org.Id, strangerId, "new@example.com",
            OrganizationRole.Member, "http://localhost:5173");
    ...
    public async Task Handle_Throws_WhenRequesterIsMemberRole()
    {
        ...
        orgRepo.GetByIdAndUserIdAsync(org.Id, memberId, Arg.Any<CancellationToken>()).Returns(org);
    ...
    public async Task Handle_CreatesInvite_SendsEmail_AndReturnsDto()
    {
        ...
        mapper.Map<InviteDto>(Arg.Any<InviteToken>())
            .Returns(new InviteDto
            {
