...
public sealed class OllamaClient : IOllamaClient
{
    ...
    public OllamaClient(HttpClient http, IModelSelector modelSelector, ILogger<OllamaClient> logger)
    {
        _http = http;
    ...
    private string CurrentModel => _modelSelector.ActiveModel;

    private string CurrentFallback
    {
        get
        {
            var active = _modelSelector.ActiveModel;
            var fallback = _modelSelector.FallbackModel;
            // Aynı modele fallback olmasın — base model'i kullan.
            return string.Equals(fallback, active, StringComparison.OrdinalIgnoreCase)
                ? _modelSelector.BaseModel
                : fallback;
        }
    }
    ...
    {
        return await GenerateWithRetryAsync(prompt, CurrentModel, jsonFormat: false, ct);
    }

    public async Task<string> GenerateJsonRawAsync(string prompt, CancellationToken ct = default)
    {
        return await GenerateWithRetryAsync(prompt, CurrentModel, jsonFormat: true, ct);
    }

    public async Task<string> ChatJsonRawAsync(IReadOnlyList<ChatMessage> messages, CancellationToken ct = default)
    {
        ...
        try
        {
            for (var attempt = 1; attempt <= MaxRetries; attempt++)
            {
                try
                {
                    return await ChatWithModelAsync(messages, model, ct);
                }
                catch (Exception ex) when (attempt < MaxRetries)
                {
                    var delay = TimeSpan.FromSeconds(Math.Pow(2, attempt));
                    _logger.LogWarning(ex, "Ollama chat attempt {Attempt}/{Max} failed, retrying in {Delay}s", attempt, MaxRetries, delay.TotalSeconds);
    ...
    private async Task<string> ChatWithModelAsync(IReadOnlyList<ChatMessage> messages, string model, CancellationToken ct)
    {
        var payload = new
        {
            model,
    ...
    {
        var model = CurrentModel;
        var raw = await GenerateWithRetryAsync(prompt, model, jsonFormat: true, ct);
        ...
        _logger.LogWarning("Primary model JSON parse failed (raw len={Len}), retrying with fallback model {Model}. Raw start: {Snippet}",
            raw.Length, fallback, raw.Length > 200 ? raw[..200] : raw);
        var rawFallback = await GenerateWithRetryAsync(prompt, fallback, jsonFormat: true, ct);
